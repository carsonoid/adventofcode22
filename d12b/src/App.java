...
public class App {
    ...
    public static void main(String[] args) throws Exception {
        ...
        var startPoints = new ArrayList<WeightedNode<String>>();
        for (var cell : grid) {
            if (cell.Value.equals("a")) {
                startPoints.add(cell);
            }
        }

        var lowest = Integer.MAX_VALUE;
        for (var sp : startPoints) {
            var dist = trySearchFrom(lines, sp.X, sp.Y);
    ...
    public static int trySearchFrom(List<String> lines, int X, int Y) {
        grid = new Grid<WeightedNode<String>>(lines, (x, y, val) -> new WeightedNode<String>(x, y, val));

        var start = grid.getCell(X, Y);
        ...
        throw new RuntimeException("No distance found");
    }
